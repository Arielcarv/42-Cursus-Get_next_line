...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, &line, 0) < 0)
		return (NULL);
	static_buffer[fd] = read_file_to_buffer(fd, static_buffer[fd]);
	...
	line = NULL;
	line = extract_line_from_buffer(static_buffer[fd]);
	static_buffer[fd] = update_static_buffer(static_buffer[fd]);
	return (line);
}
